...
def test_ejercicio_01_idempotent(x, y):
    assert maximo_basico(maximo_basico(x, y), maximo_basico(y, x)) == maximo_basico(
        y, x
    )
    assert maximo_libreria(
        maximo_libreria(x, y), maximo_libreria(y, x)
    ) == maximo_libreria(y, x)
    assert maximo_ternario(
        maximo_ternario(x, y), maximo_ternario(y, x)
    ) == maximo_ternario(y, x)


@given(
    st.floats(allow_nan=False, allow_infinity=False, max_value=0, exclude_max=True),
